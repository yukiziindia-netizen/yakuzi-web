...
export default function PremiumBrandsMegaMenu({ isOpen, onMouseEnter, onMouseLeave }: PremiumBrandsMegaMenuProps) {
  const brandGroups = [
    ...
    },
    {
      ...
      headerCls: 'text-primary bg-primary/10',
      subgroups: [
        {
    ...
    },
    {
      ...
      headerCls: 'text-secondary bg-secondary/10',
      subgroups: [
        {
        ...
        >
          <div className="w-[92vw] bg-white/80 backdrop-blur-3xl rounded-[48px] shadow-[0_48px_96px_-24px_rgba(0,0,0,0.18)] border border-white/60 overflow-hidden group">
                  ...
                  <div key={idx} className="space-y-12">
                    <div>
                      <div className={`inline-flex items-center px-4 py-2 rounded-2xl mb-10 border border-current/10 ${group.headerCls}`}>
                         <span className="text-2xs font-bold uppercase tracking-[0.2em]">{group.title}</span>
                      </div>
                      <div className="space-y-12">
                        {group.subgroups.map((sub, sIdx) => (
                          <div key={sIdx} className="group/item">
                            <h4 className="text-sm font-bold text-gray-900 mb-6 flex items-center justify-between group-hover/item:text-black transition-colors uppercase tracking-widest">
                              {sub.name}
                            ...
                            <ul className="grid grid-cols-1 gap-3.5">
                              {sub.links.map((link, lIdx) => (
                                <li key={lIdx}>
                                  <a href="#" className="text-sm font-bold text-gray-400 hover:text-gray-950 transition-all duration-300 flex items-center gap-3">
                                    <div className="w-1 h-1 rounded-full bg-gray-200 group-hover/item:bg-secondary transition-colors" />
                                    {link}
                                  </a>
                                </li>
                              ))}
              ...
              <div className="w-[520px] flex gap-12 border-l border-white/60 pl-20">
                    ...
                    <div key={idx} className="space-y-6">
                      ...
                      <ul className="space-y-4">
                        {section.links.map((link, lIdx) => (
                          <li key={lIdx}>
                            <a href="#" className="text-sm font-bold text-gray-400 hover:text-gray-950 flex items-center group/link">
                              {link}
                ...
                <div className="flex-1 grid grid-cols-2 gap-6">
                  {highlights.map((product, idx) => (
                    ...
                    >
                      <span className="absolute top-4 right-4 text-2xs font-bold uppercase tracking-[0.2em] px-2 py-1 bg-secondary text-white rounded-lg shadow-sm z-10">{product.tag}</span>
                      ...
                      <div className="text-center">
                         <p className="text-sm font-bold text-gray-900 mb-1 uppercase tracking-tight">{product.name}</p>
            ...
            <div className="bg-gray-950 p-6 flex items-center justify-center gap-8">
               <p className="text-2xs font-bold text-white/40 uppercase tracking-[0.4em]">Official Fulfillment Partner for 12,000+ Pharmacies</p>
               <div className="h-1 w-1 rounded-full bg-white/20" />
               <p className="text-2xs font-bold text-secondary uppercase tracking-[0.4em]">99.8% SLA COMPLIANCE</p>
            </div>
          </div>
        </motion.div>
      )}
