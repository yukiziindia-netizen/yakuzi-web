...
export async function setSellerSelfShip(sellerId: string, selfShipEnabled: boolean) {
  const { data } = await apiClient.patch<{ data: any }>(`/admin/sellers/${sellerId}/self-ship`, { selfShipEnabled });
  return data.data;
}
